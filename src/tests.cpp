...
//this class was used as test class during development and contains alot of small tests that test indiviual parts of the implementation
//the general idea was to test dijkstra heavily and then test the other shortest path algorithms up against dijkstra

//creates another toy example from memory
...
//tests the datastructure is correctly set up using a toy example
...
//test that reading an adjacenylist from file gives a graph that can handle SSP
void testAdjlistSimpleDijkstra(){
    ...
    assert(idvec[2]==4);
    assert(idvec[3]==5);
    cout << "Adjlist from file, Dijkstra test passed" << endl;
}
//runs handpicked tests on malta for visual evaluation
void runMaltaTests(){
...
//deprecated method used for testing the reduced graph on malta
...
//runs handpicked tests on denmark for visual evaluation
...
//test the checks that the landmarks generated by initlandmarks is non empty.
